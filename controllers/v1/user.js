...
exports.changeRole = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: 'نقش کاربر با موفقیت تغییر کرد', user: userObject });
    } catch (error) {
        return res.status(500).json({ message: "خطا در سرور" });
    }
}
...
exports.updateUser = async (req, res) => {
    try {
        ...
        // update user with id
        const user = await userModel.findOneAndUpdate({ _id: id }, { full_name, username, email, phone_number, image_url }, { new: true });
        if (!user) {
            return res.status(404).send({ message: 'کاربر مورد نظر یافت نشد' });
        }
        const userObject = passwordUtils.removeOnePropertyInObject(user, 'password');

        return res.status(200).json({ message: 'کاربر با موفقیت ویرایش شد', user: userObject });
    } catch (error) {
        return res.status(500).json({ message: "خطا در سرور" });
    }
}